...
namespace CelestialEngine.Core
{
    ...
    /// <summary>
    /// This class intelligently manages <see cref="SpriteBatch"/> so that calls to Begin() and End() will not
    ...
    internal sealed class IntelligentSpriteBatch : IDisposable
    {
        ...
        {
            this.spriteBatch.Draw(texture, this.renderSystem.GameWorld.GetPixelFromWorld(position), sourceRectangle, color, rotation, origin, scale, effects, 0);
        }
